...
/*
static void	hook_player_position(void *ptr)
{
	t_struct *game_struct = ptr;
	printf("%i %i", game_struct->player_x, game_struct->player_y);
...
void	check_escape(t_struct *game_struct)
{
		...
		[game_struct->player_x] == 'e')
	{
		...
		{
			ft_printf("Congrats! You have escaped!\n");
			exit_game(game_struct);
		}
		else
			ft_printf("Collect all the items to exit the level!\n");
	}
}
